...
- **The draft guard checks claims, not phrasing.** Every number, unit, date, name, address, link and content word in a proposed reply must be grounded in verified document evidence, an entity record, the company profile, a company rule, or the platform's own letter vocabulary. `500 kg` is refused when 500 pieces were verified, `15 novembre` when the 15th of October was, `3 settimane` when three working days were promised, and `impianto`, `magazzino`, `stabilimento` or `contratto` are refused unless the message or the company's own configuration contains them. What it still cannot judge is whether a _grounded_ sentence is a faithful reading of the facts: it checks that every word and value has a source, not that the sentence they form is true. Deciding entailment is not something deterministic code can do, and no model is given that authority.
- **An internal expectation is not a promise.** `commercial_inbox.quotation_lead_time_days` is a planning figure and never reaches the writer. Only `commercial_inbox.quotation_customer_commitment_days` authorises a reply to state a deadline; with it unset, a reply says an answer will follow after the internal review and names no date.
- **A reply is sent at least once, with one identity.** An approval commits a durable entitlement to act, and a worker carries it out under a `SELECT … FOR UPDATE` lock, so two workers never both send and a retry after success does nothing. What cannot be closed is the window between the provider accepting a message and the transaction committing: a crash there delivers the mail and rolls the record back, and the retry sends again. Every attempt carries the same `Message-ID`, derived from the approved action, so a duplicate arrives as one message that conforming clients collapse. Exactly-once delivery across SMTP and PostgreSQL is not achievable and DOLMIR does not claim it; exactly-once _persisted outcome_ is guaranteed.
