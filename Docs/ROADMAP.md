...
**Amendment 3 (2026-07-29) — Phase 2A gains the ReasoningSession orchestration layer and the full canonical stage-base set (Sprint 2 "Reasoning Kernel").** A sprint requested a "Reasoning Kernel" under a linear-pipeline vocabulary (`PipelineExecutor`, `PipelineStage`, `StageResult`, `StageFailure`, `PipelineTrace`, `CognitiveContext`, `ConfidenceAccumulator`). Per the frozen Core Architecture §8 — "an explicit directed graph of typed nodes executed by a graph executor, *not a hardcoded linear function*" — that vocabulary is a rename of the already-built typed-graph engine (`GraphExecutor`/`GraphNode`/`NodeReport`/`NodeFailure`/`ReasoningTrace`/`GraphContext`), and forking it into a parallel linear stack would violate "don't redesign / single source of truth". Resolution (recorded rather than silently applied): the engine is **not** forked; the Sprint-2 names map onto the frozen kernel (documented in `src/dolmir/orchestration/README.md`). What was genuinely missing and is added here: **`ReasoningSession`** (the injected orchestration entry point that runs a graph, persists its trace, renders the explanation, and returns an immutable **`CognitiveState`** snapshot), plus the remaining canonical fast-loop stage bases (`PerceptionNode`, `ContextBuildingNode`, `WorldModelUpdateNode`, `HypothesisGenerationNode`, `RiskEvaluationNode`, `DecisionNode`) and the `AssembledContext` type — so all twelve CogA §3 fast-loop stages have reusable homes. The Slow Learning Loop remains a placeholder (Phase 8).

**Amendment 2 (2026-07-11) — Phase 2A extended with the complete cognitive object vocabulary.** At the product owner's direction, Phase 2A's deliverables grew to cover the *entire* universal reasoning vocabulary — including the perception front (`Observation`, `ObservationSet`, `Interpretation` + an `InterpretationNode` stage base), explicit two-kind `Uncertainty` (aleatory/epistemic, CogA §5), the generic risk-gated `Decision` layer (`IdentifiedRisk`, `RiskAssessment`, `Decision`: action over unacceptable risk is unconstructible — the generalized form of the trading Risk Gate), the slow-loop shapes (`Reflection` + a `ReflectionNode` stage base, `LearningSignal` — shapes only; the producing stages remain Phase 8, which needs real outcomes), the durable-understanding shapes (`Belief` with mandatory provenance and append-only revision, generic `WorldModel` — the type `InstrumentWorldModel` and `TraderProfile` will specialize per CogA §8; persistence remains Memory Engine's, Phases 5/12), and a serializability contract (`to_document`: every reasoning object renders to self-describing JSON carrying `schema_version`; deserialization/upcasting remains with the first persistence adapter). Rationale: completing the vocabulary before the first LLM consumer (2B) means no cognitive concept gets invented ad-hoc inside a trading prompt later. Two concepts are deliberately mappings, not classes — Fact (`Claim` + grounding rule) and Counter-Hypothesis (competing set members + `Challenge`) — documented in `src/dolmir/orchestration/README.md`.
